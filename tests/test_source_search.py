...
def test_source_search_parser_extracts_french_filters() -> None:
    parsed = parse_source_search_query("articles français publiés par Le Monde")

    assert parsed.subject_query == ""
    assert parsed.language is not None
    assert parsed.language.value == "fr"
    assert parsed.publisher_name is not None
    assert parsed.publisher_name.value == "Le Monde"
...
def test_source_search_parser_extracts_author_and_date() -> None:
    parsed = parse_source_search_query("articles écrits par Ada Lovelace après janvier 2026")

    assert parsed.subject_query == ""
    assert parsed.author_name is not None
    assert parsed.author_name.value == "Ada Lovelace"
    assert parsed.published_from is not None
    assert parsed.published_from.value == datetime(2026, 1, 1, tzinfo=UTC)
...
def test_vector_ranking_deduplicates_and_uses_date_tiebreaker() -> None:
    sparse = [
        QdrantScoredArticleEmbeddingPointRead(point_id="1", article_id=1, article_key="a", model_name="BAAI/bge-m3", score=0.9, published_at=datetime(2026, 1, 1, tzinfo=UTC)),
        QdrantScoredArticleEmbeddingPointRead(point_id="2", article_id=2, article_key="b", model_name="BAAI/bge-m3", score=0.8, published_at=datetime(2026, 2, 1, tzinfo=UTC)),
    ]
    dense = [
        QdrantScoredArticleEmbeddingPointRead(point_id="2", article_id=2, article_key="b", model_name="BAAI/bge-m3", score=0.4, published_at=datetime(2026, 2, 1, tzinfo=UTC)),
        QdrantScoredArticleEmbeddingPointRead(point_id="3", article_id=3, article_key="c", model_name="BAAI/bge-m3", score=0.3, published_at=datetime(2026, 3, 1, tzinfo=UTC)),
    ]

    result = _rank_vector_candidates(sparse_items=sparse, dense_items=dense)

    assert {candidate.article_id for candidate in result} == {1, 2, 3}
